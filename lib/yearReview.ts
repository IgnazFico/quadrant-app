...
export async function getYearReview(userId: string, year: number) {
  ...
  const overallRate = totalGoals ? Math.round((completedCount / totalGoals) * 100) : 0;
  ...
  const busiestMonth = hasAnyMonth ? MONTH_NAMES[monthCounts.indexOf(Math.max(...monthCounts))] : null;
  ...
  const mostRecentCompleted = sortedByDate.length
    ? { title: sortedByDate[sortedByDate.length - 1].title, date: sortedByDate[sortedByDate.length - 1].completedAt }
    : null;

  // ---------------- role insights ----------------
  let mostImproved: any = null;
  for (const r of roles) {
    const h1Goals = r.goals.filter((g) => g.weekStart >= start && g.weekStart < midYear);
    const h2Goals = r.goals.filter((g) => g.weekStart >= midYear && g.weekStart < end);
  ...
  for (const r of roles) {
    ...
    if (!longestHeld || tenureYears > longestHeld.tenureYears) {
      longestHeld = { roleId: r.id, label: r.label, domain: r.domain, tenureYears };
    }
  }
  ...
  for (const r of roles) {
    ...
    if (!quietest || r.goals.length < quietest.goalCount) {
      quietest = { roleId: r.id, label: r.label, domain: r.domain, goalCount: r.goals.length };
    }
  }
  ...
  return {
    ...
    roleInsights: { mostImproved, mostConsistent, longestHeld, quietest, newThisYear },
    ...
    missionStatement: latestStatement
      ? {
          ...
          contentEncrypted: latestStatement.contentEncrypted.toString("base64"),
        }
      : null,
  };
}
